<!DOCTYPE html>
<html lang="ru">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Окна+</title>
    <link rel="stylesheet" href="./css/normalize.css">
    <link rel="stylesheet" href="./css/main.css">
    <link rel="stylesheet" type="text/css" href="slick/slick.css"/>
    <link rel="stylesheet" type="text/css" href="slick/slick-theme.css"/>
</head>
<body>

    <div class="page1" id='1'>
        <div class="container">
            <header class="header">
                <a href="#" class="logo">Окна+</a>
                <nav class='animate'>
                    <a href="#2" class="nav">Портфолио</a>
                    <a href="#3" class="nav">Услуги</a>
                    <a href="#4" class="nav">О нас</a>
                    <a href="#5" class="nav">Контакты</a>
                </nav>
                <div class="numberdiv1">
                    <div class="numberdiv2">
                        <a href="#" class="tel">8(800)777-77-77</a>
                        <span>Пн-Пт: 8:00 - 17:00</span>
                    </div>
                    <a href="#" class="number">Позвонить</a>
                </div>
            </header>
            <div class="page1-content">
                <div class="page1-content-rigth">
                    <h1 class="page1-h1">Пластиковые окна в Коврове</h1>
                    <h2 class="page1-h2">Производство и установка под ключ.</h2>
                    <h3 class="page1-h3">Мы занимаеся производством и установкой пластиковых окон уже <span>больше двух лет.</span> За все время работы мы не получали ни одного плохого отзыва, что говорит о нашей проффесиональности.</h3>
                    <h3 class="page1-h3">Вызов мастера для замера и доставка бесплатно в пределах города.</h3>
                    <div class="page1-offerdiv animate">
                        <a href="#5" class="page1-offer">Оставить заявку</a>
                    </div>
                </div>
                <div class="page1-content-img">
                    <img src="./img/white-2563976_1920.jpg" alt="окно" class="page1-img">
                </div>     
            </div>
        </div>
    </div>

    <div class="page2" id='2'>
        <div class="container">
            <div class='page2-info'>
                <span class='page2-portfolio'>Портфолио</span>
                <span class='page2-pictures'>Фото наших работ</span>
            </div>
            <div class='page2-slider'>
                <div class="page2-img"><img src="img/modern-building-against-blue-sky-323776 1123.png" alt=""></div>
                <div class="page2-img"><img src="img/modern-building-against-blue-sky-323776 1123.png" alt=""></div>
                <div class="page2-img"><img src="img/modern-building-against-blue-sky-323776 1123.png" alt=""></div>
                <div class="page2-img"><img src="img/modern-building-against-blue-sky-323776 1123.png" alt=""></div>
                <div class="page2-img"><img src="img/modern-building-against-blue-sky-323776 1123.png" alt=""></div>
            </div>
        </div>
    </div>

    <div class="page3" id='3'>
        <div class="container">
            <div class='page3-info'>
                <span class='page3-order'>Наши услуги</span>
                <span class='page3-suggest'>Мы предоставляем</span>
            </div>
            <div class="page3-product">
                <div class="page3-rubbish">
                    <span class="wold">Вывоз мусора</span>
                    <span class="ligth">500 руб</span>
                    <span class="ligth">(бесплатно, если под ключ)</span>
                </div>
                <div class="page3-repair">
                    <span class="wold">Обслуживание окон</span>
                    <span class="ligth">1000 руб</span>
                    <span class="ligth">(1 раз в год бесплатно, если ставили мы)</span>
                </div>
                <div class="page3-replacement">
                    <span class="wold">Замена стекла</span>
                    <span class="ligth">500 руб</span>
                </div>
                <div class="page3-orders animate"><a href="#5" class="order">Оставить заявку</a></div>
            </div>
        </div>
    </div>

    <div class="page4" id='4'>
        <div class="container">
            <div class="page4-info">
                <span class="page4-about">О нас</span>
                <span class="page4-our">Наши преимущества</span>
                <span class='border'></span>
            </div>
            <div class="page4-advantages">
                <div class="advantages-1">
                    <img src="img/advantages/001-piggy-bank 1.png" alt="" class='ourimg'>
                    <div>
                        <span class='page4-one'>Доступные цены</span>
                        <span class='page4-two'>Пластиковые окна от производителя, без посреднеческих накруток.</span>
                    </div>
                </div>
                <div class="advantages-2">
                    <img src="img/advantages/002-stopwatch 1.png" alt="" class='ourimg'>
                    <div>
                        <span class='page4-one'>Быстрое изготовление и установка</span>
                        <span class='page4-two'>Оперативность выполнения заказов за счет собственного производства и отлаженных процессов.</span>
                    </div>
                </div>
                <div class="advantages-3">
                    <img src="img/advantages/003-work-time 1.png" alt="" class='ourimg'>
                    <div>
                        <span class='page4-one'>Собственное производство</span>
                        <span class='page4-two'>Современные европейские материалы высокого качества, современные станки и автоматизированные линии сборки изделий.</span>
                    </div>
                </div>
                <div class="advantages-4">
                    <img src="img/advantages/004-feedback 1.png" alt="" class='ourimg'>
                    <div>
                        <span class='page4-one'>Высокое качество</span>
                        <span class='page4-two'>Много положительных отзывов, что говорит о высоком качестве изделий, а также о квалификации установочной бригады.</span>
                    </div>
                </div>
            </div>

        </div>
    </div>


    <div class="page5" id='5'>
        <div class="container">
            <div class="page5-info">
                <span class='page5-about'>Контакты</span>
                <span class='page5-our'>Оставьте заявку, мы вам перезвоним</span>
            </div>
            <div class="page5-input">
                <form action="mail.php" method="POST">
                    <div class="page5-input-fio">
                        <input type="text" class = 'inputsub' placeholder="Введите Ваше ФИО" maxlength="40" required name='fio'>
                        <img src="img/submit/001-user.png" alt="" class="fio">
                    </div>
                    <div class="page5-input-fio">
                        <input type="tel" class = 'inputsub' placeholder="Введите Ваш телефон"  pattern="[0-9]{11}" maxlength="11" required name='tel'>
                        <img src="img/submit/003-telephone.png" alt="" class="fio">
                    </div>
                    <div class="page5-input-fio">
                        <input type="text" class = 'inputsub' placeholder="Введите Ваш e-mail (необязательно)" maxlength="40" name='email'>
                        <img src="img/submit/002-envelope.png" alt="" class="fio">
                    </div>
                    <div class="page5-submit"><input type="submit" value="Отправить" class='submit'></div>
                </form>
            </div>
        </div>
    </div>

    <footer class='footer'>
        <div class="container">
            <nav id="nav" class='footer-nav animate'>
                <a href="#2" class="nav">Портфолио</a>
                <a href="#3" class="nav">Услуги</a>
                <a href="#4" class="nav">О нас</a>
                <a href="#5" class="nav">Контакты</a>
            </nav>
            <div class="footer-br"></div>
            <div class="footer-info">
                <div>
                    <span>2017 - 2020</span>
                    <span>г. Ковров</span>
                </div>
                <div>
                    <span>тел: 8(800)777-77-77</span>
                    <span>email: onemedesu@gmail.com</span>
                </div>
                <a href="#" class="number">Позвонить</a>
            </div>
        </div>
    </footer>


    <script src="js/jquery.min.js"></script>
    <script src="slick/slick.min.js"></script>
    <script src="js/main.js"></script>
</body>
</html>